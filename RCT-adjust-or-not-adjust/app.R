                ...
                sidebarLayout(
                    ...
                    sidebarPanel( width=3 ,
                                  ...
                                  div(
                                      ...
                                      splitLayout(
                                          textInput("v1", div(h5(tags$span(style="color:blue", "Treatment 2 arms"))), value= "1"),
...
server <- shinyServer(function(input, output   ) {
    ...
    design <- reactive({
        ...
        z        <- sample(1:2,   n, replace=TRUE)      # trt 3 levels
       ...
       # da <- data.frame(cbind(  trt ,  smoking, age, covar3, covar1, vas, time, covar2, fact1, binary2, sex, bmi))
        ...
        da <- data.frame(cbind(  z ,  smoking, age, covar3, covar1, vas, time, covar2, fact1, binary2, sex, bmi))
        # dd <<- datadist(da)
        # options(datadist="dd")
        ...
        return(list(  da=da ,
                      ...
                      trt.coef       =trt.coef  ,   # log odds ratio so 1 -> 2.718, so 1 is LARGE
                      age.coef       =age.coef   ,   # log odds of 1 over the age range
                      smoke.coef     =  smoke.coef,     # this is odds of 1.5
                      bmi.coef       =  bmi.coef   ,   # this is an odds of 1..50:50
                      covar3.coef    =  covar3.coef ,    # log odds 1 over range of 3
                      covar1.coef    =  covar1.coef  ,   # log odds -.05 per unit change
                      vas.coef       =  vas.coef,     #, log odds .008 per unit change. log odds .25 over 30 units odds 1.27
                      time.coef      =  time.coef,     # log odds -.01 per year, log odds -.1 over 10 years or odds .90
                      covar2.coef    =  covar2.coef,     # log odds 0.02 per joint, log odds 1 over 50 units or odds 2.7
                      fact1.coef     =  fact1.coef  ,  # log odds 0.693 per change in binary, or odds of 2   
                      binary2.coef   =  binary2.coef ,   # log odds 0 per change in binary, or odds of 1  
                      sex.coef       =  sex.coef    # log odds -0.         
    ...
    #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    # LOGISTIC SIMULATION CODE STARTS HERE
    #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
   ...
   Lsimul <- reactive({
        ...
        covar3.coef    =  design()$covar3.coef   # log odds 1 over range of 3
        covar1.coef    =  design()$covar1.coef   # log odds -.05 per unit change
        vas.coef       =  design()$vas.coef     #, log odds .008 per unit change. log odds .25 over 30 units odds 1.27
        time.coef      =  design()$time.coef     # log odds -.01 per year, log odds -.1 over 10 years or odds .90
        covar2.coef    =  design()$covar2.coef     # log odds 0.02 per joint, log odds 1 over 50 units or odds 2.7
        fact1.coef     =  design()$fact1.coef   # log odds 0.693 per change in binary, or odds of 2   
        binary2.coef   =  design()$binary2.coef   # log odds 0 per change in binary, or odds of 1  
        sex.coef       =  design()$sex.coef    # log 
      ...
      #  b1 <- round(sort(runif(K1, -theta1*Fact,theta1*Fact)), digits=2) 
           lp = intercept + z*trt.coef + smoking*smoke.coef + age*age.coef  + bmi*bmi.coef + covar3*covar3.coef +
            ...
            lp = intercept + z*trt.coef  
            y2 <- ifelse(randomi < plogis(lp), 1, 0)   # one liner RANDOM!!!
            
            lp <- intercept + z*trt.coef + smoking*smoke.coef + age*age.coef  + bmi*bmi.coef  # just have 3 variables associated with response
            y3 <- ifelse(randomi < plogis(lp), 1, 0)   # one liner RANDOM!!!
            
            data.frame( y=y,  y2=y2, y3=y3, da)
        ...
        Lstatfun <- function(d) {
            ...
            zz5 <- glm(y3~z, data=d,   family = "binomial")         ## not adjusting some X  are prognostic
            f5 <-  summary(zz5)
            #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~collect estimates
            ...
            cbind(
                ...
                mean(quantile( sum(zz$residuals^2) / zz$df.residual , .025)), #25
                mean(quantile( sum(zz$residuals^2) / zz$df.residual , .975)), 
                mean(quantile( sum(zz1$residuals^2) / zz1$df.residual , .025)), #25
                mean(quantile( sum(zz1$residuals^2) / zz1$df.residual , .975)), 
                mean(quantile( sum(zz2$residuals^2) / zz2$df.residual , .025)), #25
                mean(quantile( sum(zz2$residuals^2) / zz2$df.residual , .975)), 
                mean(quantile( sum(zz3$residuals^2) / zz3$df.residual , .025)), #25
                mean(quantile( sum(zz3$residuals^2) / zz3$df.residual , .975)), 
                mean(quantile( sum(zz4$residuals^2) / zz4$df.residual , .025)), #25
                mean(quantile( sum(zz4$residuals^2) / zz4$df.residual , .975)), 
                mean(quantile( sum(zz5$residuals^2) / zz5$df.residual , .025)), #25
                mean(quantile( sum(zz5$residuals^2) / zz5$df.residual , .975)), 
        ...
        library(plyr)
        res <- raply(simuls, Lstatfun(Lsimfun())) # run the model many times
        # summarize
